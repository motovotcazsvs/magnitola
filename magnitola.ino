...
void setup() {
  ...
  delay(800);
  mp3_set_volume (volDF);
  
  lcd.setCursor(3,1);
  lcd.print(time.gettime("d.m.Y"));

  radio.init();
  radio.setMono(noStereo);
...
void DFplayer(){
  ...
  lo = lowByte(track); // младший байт
  EEPROM.update(0, hi);
  EEPROM.update(1, lo);
  lcd.setCursor(10, 1);
  lcd.print("Vol ");
  lcd.print(volDF);
  EEPROM.update(4 ,volDF);
}
...
void FMradio(){
  ...
  hi = highByte(frequency); // старший байт
  lo = lowByte(frequency); // младший байт
  EEPROM.update(2, hi);
  EEPROM.update(3, lo);
  lcd.setCursor(10, 1);
  lcd.print("      ");
...
void volMax(){
  ...
  {
    volFM = radio.getVolume();
    if (volFM < 15) radio.setVolume(++volFM);
    FMradio();
  }
}
...
void volMin(){
  ...
  {
    volFM = radio.getVolume();
    if (volFM > 0) radio.setVolume(--volFM);
    FMradio();
  }
}
